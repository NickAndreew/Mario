...
function Player:update(dt)
    if self.map.gameState ~= 'victory' and self.map.gameState ~= 'game_over' then
...
function Player:calculateJumps()
    ...
    if self.dy < 0 then
        if (
            ...
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= TILE_EMPTY
        )
        and
        (
            self.map:tileAt(self.x, self.y).id ~= CLOUD_LEFT or
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= CLOUD_LEFT
        ) 
        and
        ( 
            self.map:tileAt(self.x, self.y).id ~= CLOUD_RIGHT or
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= CLOUD_RIGHT
        )
        and 
        ( 
            self.map:tileAt(self.x, self.y).id ~= JUMP_BLOCK_HIT or
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= JUMP_BLOCK_HIT
        )
        and 
        ( 
            self.map:tileAt(self.x, self.y).id ~= FLAG_TOP or
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= FLAG_TOP
        )
        and 
        ( 
            self.map:tileAt(self.x, self.y).id ~= FLAG_MIDDLE or
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= FLAG_MIDDLE
        )
        and 
        ( 
            self.map:tileAt(self.x, self.y).id ~= FLAG_BOTTOM or
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= FLAG_BOTTOM
        ) then
            -- reset y velocity
            self.dy = 0
            ...
            if self.map:tileAt(self.x, self.y).id == JUMP_BLOCK then
                ...
                playCoin = true
                self.map.scores = self.map.scores + 1
            else
                playHit = true
            end
...
function Player:checkLeftCollision()
    if self.dx < 0 then
        ...
        if (self.map:collides(self.map:tileAt(self.x - 1, self.y)) or
            self.map:collides(self.map:tileAt(self.x - 1, self.y + self.height - 1)))
            and ((self.x - self.map.tileWidth) > 0) then
